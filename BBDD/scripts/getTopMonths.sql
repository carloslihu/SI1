...
RETURNS table(fecha text, money numeric, sales bigint) AS $$
DECLARE

BEGIN
  RETURN QUERY SELECT * from
    (SELECT to_char(orderdate, 'YYYY-MM') as fecha, sum(totalamount) as total from orders
      group by fecha) as amount
  natural join

...
BEGIN
    ...
    (SELECT f as fecha,
      ...
      FROM (
        SELECT f,
          MAX(total2) as total3
        FROM (
          SELECT f,
            movieid,
            SUM(total) as total2
          FROM (
            SELECT EXTRACT(YEAR FROM orderdate) as f, 
              prod_id, 
            ...
            GROUP BY f, 
              prod_id) AS q--años y productos y cantidad de veces que se vendio en esa fecha
      ...
      (
        ...
        FROM (
              ...
              where EXTRACT(YEAR FROM orderdate) >= anno
              GROUP BY f,
                prod_id) AS q--fechas y productos y cantidad de veces que se vendio en esa fecha
          NATURAL JOIN products
          GROUP BY f, 
            movieid) AS q4
